			...
			<td style=\"text-align: center;\">
				<div style=\"\">
					<div style=\"float: left; padding: 10px;\">
						<form action=\"./page.php?ref=$_GET[ref]&token=$_GET[token]\" method=\"post\">
							<select name=\"aksi\" onchange=\"this.form.submit()\">";
								$isi.="<option value=\"\">-Mode-</option>";
								$arr=array('2'=>'Tahunan','1'=>'Bulanan');
								foreach($arr as $key=>$val) {
									if(isset($_POST['aksi'])||isset($_SESSION['aksi'])) {
										if($_POST['aksi']==$key||$_SESSION['aksi']==$key) {
											$isi.="
				...
				if($_SESSION['aksi']==1) {
								...
								if($queryTah != null) {
									if(mysql_num_rows($queryTah)>0) {
										while($rowTah=mysql_fetch_array($queryTah)) {
											if(isset($_POST['tah'])||isset($_SESSION['tah'])) {
												if($_POST['tah']==$rowTah['thn']||$_SESSION['tah']==$rowTah['thn']) {
													$isi.="
													<option value=\"$rowTah[thn]\" SELECTED>$rowTah[thn]</option>";
													$_SESSION['tah']=$rowTah['thn'];
												}
												else {
													$isi.="
					...
					if($_SESSION['aksi']==1) {
						$sqlCart="	SELECT value, bln 
									FROM tbl_fitur 
									WHERE thn='$_SESSION[tah]' 
									AND kdKatParent='1.a' 
									GROUP BY bln";
						$xAxisName="Bulan";
						$caption="Statistik Perkembangan Penduduk Per Bulanan Tahun $_SESSION[tah]";
					}
				}
				else {				
					if($_SESSION['aksi']==2) {	
						...
						if(mysql_num_rows($queryCart)>0) {
							...
							while($rowCart=mysql_fetch_array($queryCart)){
								if($_SESSION['aksi']==2) {
									...
									if($_SESSION['aksi']==1&&$_SESSION['tah']!="") {
							...
							$cart.="</chart>";
							$isi.=renderChartHTML("./library/FusionCharts/Line.swf","",$cart,"PerkembanganPenduduk",700,400,false,false);	
						}
					}
			}
					$isi.="
